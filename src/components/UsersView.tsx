...
const UserWizard: React.FC<{ catalog: UsersCatalog; editing: ManagedUser | null; onClose: () => void; onSaved: () => Promise<void> }> = ({ catalog, editing, onClose, onSaved }) => {
  ...
  const stepValid = (index: number) => {
    if (index === 0) return form.fullName.trim().length > 1 && /^[\p{L}\p{M}\p{N}_.-]+(?: [\p{L}\p{M}\p{N}_.-]+)*$/u.test(form.username.trim().replace(/\s+/g, ' ')) && (editing ? true : form.password.length >= 8);
        ...
        {stepIndex === 0 && (
          <>
            ...
            <label className="block text-xs font-extrabold text-slate-700">اسم المستخدم
              ...
              <span className="mt-1 block text-[10px] font-medium text-slate-400">حروف عربية أو إنكليزية وأرقام و . _ - فقط، ويمكن فصل الكلمات بمسافة واحدة.</span>
            </label>
            {!editing && (
