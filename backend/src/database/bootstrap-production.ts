...
async function main() {
  ...
  if (!/^[\p{L}\p{M}\p{N}_.-]+(?: [\p{L}\p{M}\p{N}_.-]+)*$/u.test(username.trim().replace(/\s+/g, ' ')) || password.length < 16) throw new Error('Bootstrap username or password does not meet the production policy.');
