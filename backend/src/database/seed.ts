...
async function main() {
  ...
  try {
    ...
    for (const [username, role] of [['admin', admin], ['admin_dev', admin], ['nabil_manager_dev', manager], ['ahmad_manager_dev', manager], ['furqan_sales_dev', sales], ['dana_sales_dev', sales]] as const) await db.insert(userRoles).values({ userId: byUsername(username).id, roleId: role.id }).onConflictDoNothing();
    for (const [username, warehouse, isManager] of [['admin', furqan, false], ['admin', dana, false], ['admin_dev', furqan, false], ['admin_dev', dana, false], ['nabil_manager_dev', furqan, true], ['furqan_sales_dev', furqan, false], ['ahmad_manager_dev', dana, true], ['dana_sales_dev', dana, false]] as const) await db.insert(userWarehouses).values({ userId: byUsername(username).id, warehouseId: warehouse.id, isManager }).onConflictDoNothing();
