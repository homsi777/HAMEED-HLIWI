...
const permissionCodes = ['inventory.view', 'inventory.create', 'inventory.update', 'inventory.delete', 'inventory.transfer', 'inventory.adjust', 'sales.view', 'sales.create', 'sales.update', 'sales.cancel', 'sales.return', 'sales.discount', 'sales.approve', 'purchases.view', 'purchases.create', 'purchases.cancel', 'customers.view', 'customers.create', 'customers.update', 'customers.archive', 'suppliers.view', 'suppliers.create', 'suppliers.update', 'suppliers.archive', 'reports.view', 'users.view', 'users.manage', 'warehouses.view', 'warehouses.manage', 'warehouses.scope.all'];
async function main() {
  ...
  try {
    ...
    await mapPermissions(manager.id, ['inventory.view', 'inventory.create', 'inventory.update', 'inventory.transfer', 'sales.view', 'sales.create', 'sales.update', 'purchases.view', 'purchases.create', 'customers.view', 'customers.create', 'customers.update', 'suppliers.view', 'suppliers.create', 'suppliers.update', 'reports.view', 'warehouses.view']);
    ...
    for (const [username, role] of [...(localVisualPassword ? [['admin', admin]] as const : []), ['admin_dev', admin], ['nabil_manager_dev', manager], ['ahmad_manager_dev', manager], ['furqan_sales_dev', sales], ['dana_sales_dev', sales]] as const) await db.insert(userRoles).values({ userId: byUsername(username).id, roleId: role.id }).onConflictDoNothing();
    for (const [username, warehouse, isManager] of [...(localVisualPassword ? [['admin', furqan, false], ['admin', dana, false]] as const : []), ['admin_dev', furqan, false], ['admin_dev', dana, false], ['nabil_manager_dev', furqan, true], ['furqan_sales_dev', furqan, false], ['ahmad_manager_dev', dana, true], ['dana_sales_dev', dana, false]] as const) await db.insert(userWarehouses).values({ userId: byUsername(username).id, warehouseId: warehouse.id, isManager }).onConflictDoNothing();
