...
export const inventoryMovementType = pgEnum('inventory_movement_type', ['initial', 'transfer', 'sale', 'manual_sale', 'sale_cancellation', 'purchase', 'legacy_reconciliation', 'purchase_cancellation']);
...
export const inventoryItems = pgTable('inventory_items', {
  ...
  warehouseId: uuid('warehouse_id').notNull().references(() => warehouses.id, { onDelete: 'restrict' }), status: inventoryStatus('status').notNull().default('in_stock'), quantity: numeric('quantity', { precision: 14, scale: 3 }).notNull().default('1'), isManualSaleEntry: boolean('is_manual_sale_entry').notNull().default(false), imagePath: text('image_path'), notes: text('notes'), version: integer('version').notNull().default(1), archivedAt: timestamp('archived_at', { withTimezone: true }), archivedByUserId: uuid('archived_by_user_id').references(() => users.id, { onDelete: 'set null' }), createdByUserId: uuid('created_by_user_id').notNull().references(() => users.id, { onDelete: 'restrict' }), updatedByUserId: uuid('updated_by_user_id').notNull().references(() => users.id, { onDelete: 'restrict' }), ...timestamps,
}, table => [uniqueIndex('inventory_items_code_unique').on(table.code), index('inventory_items_warehouse_status_idx').on(table.warehouseId, table.status), index('inventory_items_karat_idx').on(table.karat), index('inventory_items_category_idx').on(table.category), check('inventory_items_karat_check', sql`${table.karat} in ('24','22','21','18','14')`), check('inventory_items_weights_check', sql`(${table.isManualSaleEntry} = true and ${table.grossWeightGrams} < 0 and ${table.stoneWeightGrams} <= 0 and ${table.netWeightGrams} < 0) or (${table.isManualSaleEntry} = false and ${table.grossWeightGrams} > 0 and ${table.stoneWeightGrams} >= 0 and ${table.netWeightGrams} >= 0 and ${table.grossWeightGrams} >= ${table.stoneWeightGrams})`), check('inventory_items_quantity_check', sql`(${table.isManualSaleEntry} = true and ${table.quantity} < 0) or (${table.isManualSaleEntry} = false and ${table.quantity} > 0)`), check('inventory_items_labor_check', sql`${table.laborFeeUsdPerGram} >= 0 and ${table.totalLaborFeeUsd} >= 0`)]);

export const inventoryMovements = pgTable('inventory_movements', { id: id(), inventoryItemId: uuid('inventory_item_id').notNull().references(() => inventoryItems.id, { onDelete: 'restrict' }), salesInvoiceId: uuid('sales_invoice_id').references(() => salesInvoices.id, { onDelete: 'restrict' }), purchaseInvoiceId: uuid('purchase_invoice_id').references(() => purchaseInvoices.id, { onDelete: 'restrict' }), type: inventoryMovementType('type').notNull(), fromWarehouseId: uuid('from_warehouse_id').references(() => warehouses.id, { onDelete: 'restrict' }), toWarehouseId: uuid('to_warehouse_id').references(() => warehouses.id, { onDelete: 'restrict' }), actorUserId: uuid('actor_user_id').notNull().references(() => users.id, { onDelete: 'restrict' }), note: text('note'), metadata: jsonb('metadata').notNull().default({}), createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow() }, table => [index('inventory_movements_item_created_idx').on(table.inventoryItemId, table.createdAt), index('inventory_movements_warehouse_idx').on(table.toWarehouseId), index('inventory_movements_sale_invoice_idx').on(table.salesInvoiceId), index('inventory_movements_purchase_invoice_idx').on(table.purchaseInvoiceId)]);
