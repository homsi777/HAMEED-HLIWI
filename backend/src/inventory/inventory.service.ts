...
export class InventoryService {
  ...
  async create(user: AuthIdentity, input: Record<string, unknown>) {
    this.scope.assertAccess(user, this.uuid(this.text(input.warehouseId, 'warehouseId'), 'warehouseId')); const values = this.values(input, true);
    try { const row = await this.db.transaction(async tx => {
      let created: typeof inventoryItems.$inferSelect | undefined;
      // Small gold labels need a compact number-only code. `onConflictDoNothing` makes each
      // random six-digit attempt safe even when two staff members save at the same moment.
      for (let attempt = 0; attempt < 30 && !created; attempt += 1) {
        const code = values.code || String(Math.floor(100000 + Math.random() * 900000));
        const inserted = await tx.insert(inventoryItems).values({ ...values, code, createdByUserId: user.id, updatedByUserId: user.id }).onConflictDoNothing({ target: inventoryItems.code }).returning();
        created = inserted[0];
        if (values.code && !created) throw new ConflictException('Item code already exists.');
      }
      if (!created) throw new ConflictException('Could not allocate a unique six-digit barcode. Please retry.');
      await tx.insert(inventoryMovements).values({ inventoryItemId: created.id, type: 'initial', toWarehouseId: created.warehouseId, actorUserId: user.id, note: this.optional(input.notes) });
      return created;
